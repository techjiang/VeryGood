...
**当前版本：v1.6.1** — 左侧栏固定不动、彻底移除幻灯片功能、静态资源版本戳 v=7。
...
**左侧栏保持固定不动** — 彻底移除 v1.5.x 的图片链接幻灯片功能和 v1.6.0 尝试的心电图特效，左侧栏恢复干净简洁的固定状态：`position: fixed`，不随页面滚动移动，视觉稳定。

**稳定性提升** — 不再依赖任何外部图片域名或复杂动画，不再有任何视觉特效带来的潜在问题，左侧栏始终保持干净、可靠。

**静态资源版本戳** — v=7，确保浏览器缓存正确更新。

## v1.6.0 更新内容（已回退）

v1.6.0 曾尝试在左侧栏增加纯 CSS/SVG 心电图特效，v1.6.1 已移除该特效，恢复干净简洁的左侧栏。
...
**幻灯片轮播**（已移除） — 曾在左侧栏新增图片链接幻灯片轮播功能，后因 PC 端浏览器追踪防护拦截外部图床域名导致图片不显示，v1.6.1 彻底移除。
