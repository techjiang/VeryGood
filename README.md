...
**当前版本：v1.1.5** —— 标签/分类 URL 空格修复（SEO 加固）· 页脚不再被遮挡（移入布局容器）· 回顶按钮近底自动上浮 · 滚动监听直连（去 rAF 包装）· 去掉毛玻璃模糊（性能优化）· 左右侧栏绝对固定 · 页脚署名放大 · 朋友圈动态（站长 Issue 推送）· 全站内容 Actions 一键更新 · 页脚署名防删除双保险 · 三栏布局 · 公告条 · 图片/视频灯箱 · 环形回顶进度 · 友链头像 · 可折叠侧栏。
...
site:
  ...
  # v1.1：顶部公告条（可关闭，状态存 localStorage）
  announcement:
    enabled: true               # 是否显示
    text: "🎉 欢迎来访"          # 文案（支持少量 HTML）
    link: "/board/"             # 点击跳转，留空则不可点
